...
int setShip(int ship, char direction, int x, int y, Cell map[][10]){//функци€ дл€ установовки корабл€
...
void randomShip(Cell map[][10]){//функци€ рандомной расстановки кораблей
    ...
    while(ships[1] != 0 || ships[2] !=0 || ships[3] != 0 || ships[4] != 0){// цикл прерветс€ когда все корабли закончатс€
        ...
        if (ship >= 1 && ship <= 4 && ships[ship] > 0){
                ...
                else direction = VERTICALLY;//рандомно выбираем направление корабл€
                int x, y;
                x = rand() % 10;
                y = rand() % 10;//рандомно выбираем начальные координаты корабл€
    ...
    while(ships[1] != 0 || ships[2] !=0 || ships[3] != 0 || ships[4] != 0){
        ...
        if (ship >= 1 && ship <= 4 && ships[ship] > 0){ // если есть такой корабль и он длиной от 1 до 4, то продолжаем ввод
                if (ship != 1){
                        ...
                        direction = getche(); //направление корабл€
                        printf("\n");
                        }
